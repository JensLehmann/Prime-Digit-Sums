...
from m_value import M_EMPIRICAL_LITERAL, M_RIGOROUS_CERTIFIED_INT, M_THEOREM_HEADLINE_INT
...
M_computed = result["M"]
M_from_literal = mp.mpf(M_EMPIRICAL_LITERAL)

rel_diff = abs(M_computed - M_from_literal) / M_from_literal
tol = mp.mpf("1e-39")

if rel_diff >= tol:
    print(f"FAIL: M_EMPIRICAL_LITERAL does not match fresh computation.")
    print(f"  M_EMPIRICAL_LITERAL = {M_EMPIRICAL_LITERAL}")
    print(f"  M_computed = {mp.nstr(M_computed, 40)}")
    print(f"  rel diff   = {mp.nstr(rel_diff, 5)}  (tolerance {mp.nstr(tol, 2)})")
    sys.exit(1)

print(f"OK: M_EMPIRICAL_LITERAL matches fresh computation (rel diff < 1e-39).")
print(f"  M_EMPIRICAL_LITERAL = {M_EMPIRICAL_LITERAL}")
print(f"  M_computed = {mp.nstr(M_computed, 40)}")
