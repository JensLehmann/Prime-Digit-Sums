...
| `m_value.py` | Stores the empirical diagnostic $M$ literal and the rigorous theorem-level $M$ constants. |
| `test_m_value.py` | Regression test verifying `M_EMPIRICAL_LITERAL` matches a fresh `compute_M(Inputs())` run; run directly (no pytest needed). |
