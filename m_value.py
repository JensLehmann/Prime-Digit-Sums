"""Threshold value M used by verify_q0.py and find_terminal_seeds.py.

This is the empirical value of M produced by the constraint checker at the
paper's working parameters (eta, nu) = (0.0545, 0.2859), using the script's
bisection for the characteristic-function threshold.  The theorem itself uses
the rigorous closed-form certificate Y_* <= 9.10e31 and states
M < 1.78e32.
...
and compare with M_LITERAL below.  test_m_value.py automates this check.
"""
...
# threshold.
M_LITERAL = "148288679844722033445044981196451.0e0"

# Float form for coarse diagnostics only; exact threshold comparisons must use
# M_INT or M_THEOREM_HEADLINE_INT.
M_FLOAT = float(M_LITERAL)

# Exact-integer form for bignum threshold comparisons q >= M where q is an
# exact integer.  This is the integer reading of M_LITERAL (the digits before
# the decimal point); test_m_value.py guards against drift.
M_INT = 148288679844722033445044981196451
assert M_INT == int(M_LITERAL.split(".")[0])
...
assert M_INT < M_RIGOROUS_CERTIFIED_INT < M_THEOREM_HEADLINE_INT
